...
def load_processed_dataframe(file_path: Path) -> pd.DataFrame:
    """
    Load one processed CSV.
    """
    ...
    logger.info(f"Loaded rows: {len(dataframe)}")
...
def process_dataframe(file_path: Path) -> int:
    ...
    dataframe = load_processed_dataframe(file_path)
    ...
    logger.info(f"DataFrame processed successfully: {output_file.name}\n")
